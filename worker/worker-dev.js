...
const slugs = [];
const pages = [];
// let { SLUG_TO_PAGE, PAGE_TITLE, PAGE_DESCRIPTION, DOMAIN } = SETTINGS;
...
async function gatherResponse() {
  ...
  const response = await fetch(settingsUrl, config);
  try {
...
async function fetchAndApply(request) {
  ...
  let fullPathname = request.url.replace("https://" + DOMAIN, "");
  let response;
  if (url.pathname.startsWith("/app") && url.pathname.endsWith("js")) {
    response = await fetch(url.toString());
    let body = await response.text();
    response = new Response(
      body.replace(/www.notion.so/g, DOMAIN).replace(/notion.so/g, DOMAIN),
      response
    );
...
class HeadRewriter {
  ...
  element(element) {
    ...
    element.append(
      `<meta property="og:image" content="https://raw.githubusercontent.com/orelhassid/notion/b3768a3322588ba0bb1b913abae02d3e7a54b2bb/icons/logo.svg">
<meta property="twitter:image" content="https://raw.githubusercontent.com/orelhassid/notion/b3768a3322588ba0bb1b913abae02d3e7a54b2bb/icons/logo.svg">
<link rel="shortcut icon" type="image/x-icon" href="https://raw.githubusercontent.com/orelhassid/notion/b3768a3322588ba0bb1b913abae02d3e7a54b2bb/icons/logo.svg">`,
      {
        html: true,
      }
    );
